...
async def find_recipe(ingredients: list, max_time_min: int = 60) -> str:
    matches = []
...
async def substitute_ingredient(ingredient: str) -> str:
    subs = {"butter": "olive oil", "sugar": "honey", "cream": "coconut milk"}
    return json.dumps({"original": ingredient, "substitute": subs.get(ingredient.lower(), "no substitute known")})
